...
class AddRideBloc extends Bloc {
  AddRideBloc({required this.repository});
  ...
  final StreamController<DateTime> _calendarHeaderController =
      BehaviorSubject();
  Stream<DateTime> get headerStream => _calendarHeaderController.stream;

  final StreamController<bool> _showDeleteSelectionController =
      BehaviorSubject();
  Stream<bool> get showDeleteSelectionStream =>
      _showDeleteSelectionController.stream;

  ///The date for the currently visible month in the calendar.
  late DateTime pageDate;

  final Duration _pageDuration = const Duration(milliseconds: 300);
  ...
  final _ridesToAdd = HashSet<DateTime>();
  ...
  void onClearSelection() {
    ...
    if (!_currentSubmitState.saving && !_currentSubmitState.noSelection) {
      _ridesToAdd.clear();
      for (final method in _resetFunctions) {
        method();
      }
  ...
  void registerResetFunction(VoidCallback function) =>
      _resetFunctions.add(function);

  void unregisterResetFunction(VoidCallback function) =>
      _resetFunctions.remove(function);
  ...
  void _addMonth() {
    ...
    final newDate = Jiffy([pageDate.year, pageDate.month, 1])..add(months: 1);
  ...
  Future<void> addRides() async {
    ...
    if (_ridesToAdd.isNotEmpty) {
      await repository
          .addRides(_ridesToAdd.map((date) => Ride(date: date)).toList());
    } else {
      //empty selection
  ...
  int get hashCode {
    ...
    return hashValues(pageDate, _currentSubmitState,
        setEquality.hash(_existingRides), setEquality.hash(_ridesToAdd));
  }

  @override
  bool operator ==(Object other) {
    const setEquality = SetEquality();
    return other is AddRideBloc &&
        pageDate == other.pageDate &&
