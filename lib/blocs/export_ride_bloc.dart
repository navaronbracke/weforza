...
class ExportRideBloc extends Bloc {
  ...
  void exportRide() async {
    await fileHandler
        .createFile(filename, _fileExtension.extension())
        .then((file) async {
      if (await file.exists()) {
        ...
        await _saveRideAndAttendeesToFile(file, _fileExtension.extension(),
            ride, await rideRepository.getRideAttendees(ride.date));
  ...
  Future<void> _saveRideAndAttendeesToFile(
      File file, String extension, Ride ride, List<Member> attendees) async {
    if (extension == FileExtension.CSV.extension()) {
      ...
      for (Member m in attendees) {
        buffer.writeln(ExportableRideAttendee(
                firstName: m.firstname, lastName: m.lastname, alias: m.alias)
            .toCsv());
      }
