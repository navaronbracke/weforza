...
class AnimatedCircleCheckmark extends StatefulWidget {
  const AnimatedCircleCheckmark({required this.controller, super.key});
  ...
  static const Duration kCheckmarkAnimationDuration = Duration(milliseconds: 300);
  ...
  Widget _buildAnimation(Color backgroundColor, IconData checkmarkIcon) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final double checkmarkCircleSize = max(constraints.biggest.shortestSide * 0.3, 240);
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: AnimatedBuilder(
        animation: widget.controller.drive(CurveTween(curve: Curves.easeOutQuart)),
        builder: (context, child) => _buildAnimation(backgroundColor, checkmarkIcon),
      ),
    );
  }
}
