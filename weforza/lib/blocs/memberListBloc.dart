...
class MemberListBloc extends Bloc {
  ...
  Future<MembersListWithFilterModel>? membersFuture;

  /// Load the members.
  /// This function first does a lookup for the settings that contain the specified filter value.
  void loadMembers() {
    membersFuture = _settingsRepository.loadApplicationSettings().then((settings) async {
      return MembersListWithFilterModel(
        items: await _repository.getMembers(settings.memberListFilter),
        filter: settings.memberListFilter,
      );
    });
  }
